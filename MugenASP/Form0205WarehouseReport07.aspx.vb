...
ErrHand:
    ...
    Sub StockUnitBelumKirimSudahDO(ByVal mLokasi As String)
        ...
        Dim mSqlCommadstring As String = _
                    ...
                    "SELECT STOCK_NORANGKA " & _
                    "FROM TRXN_STOCK LEFT OUTER JOIN TRXN_SPK ON TRXN_STOCK.STOCK_NoRangka = TRXN_SPK.SPK_NoRangka WHERE " & _
                    "NOT(SPK_NODO IS NULL OR SPK_NODO='') AND STOCK_KirimTglTerima IS NULL AND YEAR(SPK_TGLDO) >= 2018 and " & _
                    "(SELECT COUNT(DBNOTAINT_NORANGKA) as mJmlDBNOTA FROM TRXN_DBNOTAINTERNAL WHERE DBNOTAINT_NORANGKA = STOCK_NoRangka)=0 " & _
                    "ORDER BY SPK_TGLDO,STOCK_CDTYPE,STOCK_CDWARNA,STOCK_TglDOImoraHpm,STOCK_NORANGKA"
        Call UpdateDatabase_Tabel(mSqlCommadstring, mLokasi)
    End Sub
