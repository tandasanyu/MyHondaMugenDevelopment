...
Partial Class Form0205Warehouse
            ...
            If MyRecReadA.HasRows = True Then
                While MyRecReadA.Read()
                    ...
                    If nSr((MyRecReadA("STOCKK_JENIS"))) = "1" Then
                        LblKirimJenis.Text = "CUSTOMER"
                        DropDownListJnsKirim.SelectedIndex = 0
                        DropDownListJnsKirim.Text = "CUSTOMER"
                    ElseIf nSr((MyRecReadA("STOCKK_JENIS"))) = "2" Then
                        LblKirimJenis.Text = "PAMERAN"
                        DropDownListJnsKirim.SelectedIndex = 1
                        DropDownListJnsKirim.Text = "PAMERAN"
                    ElseIf nSr((MyRecReadA("STOCKK_JENIS"))) = "3" Then
                        LblKirimJenis.Text = "SHOWROOM"
                        DropDownListJnsKirim.SelectedIndex = 2
                        DropDownListJnsKirim.Text = "SHOWROOM"
                    End If
    ...
    Function Insert_KirimKendaraan(ByVal mNoRangkaku As String) As Byte
        ...
        End If
        If LblKirimNo.Text <> "" Then
            Insert_KirimKendaraan = UpdateDatabase_Tabel("INSERT INTO TRXN_STOCKKKIRIM (STOCKK_NOKIRIM,STOCKK_JENIS,STOCKK_BIAYA,STOCKK_NORANGKA,STOCKK_TOMUGEN,STOCKK_STATUS,STOCKK_ADMIN) " & _
                                                         "VALUES ('" & LblKirimNo.Text & "','" & jnskirim & "',0,'" & mNoRangkaku & "','" & TxtKirimDealer.Text & "','','" & Left(LblUserName.Text, 10) & "')", "", 1)
            If Insert_KirimKendaraan = 1 Then
                TxttglKirimSetujui.Text = lblTglAdmin.Text
                If jnskirim = "1" Then
                    Insert_KirimKendaraan = UpdateDatabase_Tabel("UPDATE TRXN_STOCKMKIRIM SET STOCKM_NOKIRIM='" & LblKirimNo.Text & "',STOCKM_KIRIMTGLSETUJUI=" & FieldTgl(lblTglAdmin.Text) & " WHERE STOCKM_NORANGKA='" & mNoRangkaku & "'", "", 1)
                End If
            End If
            LvUnitKirimPerRangka.DataBind()
        Else
...
ErrHand:
    ...
    Function UPDATE_DATA_STOCK_STATUS() As Byte
        Dim Mytxtsql As String = "UPDATE TRXN_STOCK SET " & _
                   "STOCK_BlockPickup='" & IIf(CheckBoxBlokSales.Checked = True, "1", "0") & "'," & _
                   "STOCK_StsUpdate='N' " & _
                   "WHERE STOCK_NoTTK='" & TxtNoTTK.Text & "' AND STOCK_StsUpdate='P'"
