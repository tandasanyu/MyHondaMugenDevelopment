...
Partial Class Form0205WarehouseReport06
...
ErrHand:
    ...
    Sub TerimaUnit(ByVal mQuery As String)
        Dim mpServer As String = "MyConnCloudDnet"
        Dim strconn As String = WebConfigurationManager.ConnectionStrings(mpServer).ConnectionString
        Dim cnn As OleDbConnection
        Dim cmd As OleDbCommand
        Dim mSqlCommadstring As String = _
                     "SELECT STOCK_TglTTK,STOCK_NoTTK,WARNA_Int,TYPE_Nama,STOCK_NoRangka,STOCK_NoMesin,LOKASI_Nama,STOCK_TglDoDealer,STOCK_DOSPKKota,STOCK_DOTGLGESEKRGK,STOCK_DOTGLGESEKRGKI,LOKASI_KODE,LOKASI_NAMA,SUPLIER_KODE,SUPLIER_NAMA " & _
                     "FROM TRXN_STOCK,DATA_WARNA,DATA_TYPE,DATA_SUPLIER,DATA_LOKASI WHERE " & _
            ...
            If MyRecReadA.HasRows = True Then
                While MyRecReadA.Read()
                    insertTabelBBM(1, nSr(MyRecReadA("STOCK_NoTTK")), nSr(MyRecReadA("STOCK_TglTTK")), nSr(MyRecReadA("WARNA_Int")), _
                                      ...
                                      nSr(MyRecReadA("LOKASI_KODE")) & "-" & nSr(MyRecReadA("LOKASI_NAMA")), nSr(MyRecReadA("SUPLIER_KODE")) & "-" & nSr(MyRecReadA("SUPLIER_NAMA")), nSr(MyRecReadA("STOCK_DOTGLGESEKRGKI")))
                End While
            End If
            MyRecReadA.Close()
            cmd.Dispose()
    ...
    Protected Sub Button2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button2.Click
        If txtTglStart.Text <> "" And txtTglEnd.Text <> "" Then
            ...
            Call TerimaUnit("datediff(day," & DtInSQL(CDate(txtTglStart.Text)) & ",STOCK_TglTTK) >= 0 and datediff(day,STOCK_TglTTK," & DtInSQL(CDate(txtTglEnd.Text)) & ") >= 0")
        End If
    End Sub
