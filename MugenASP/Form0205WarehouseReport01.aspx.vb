...
Partial Class Form0205WarehouseReport01
    ...
    Protected Sub DefinisTabelBBM()
        ...
        dt.Columns.AddRange(New DataColumn(16) {New DataColumn("Temp_KirimRangka"), New DataColumn("Temp_KirimInput"), New DataColumn("Temp_KirimMohon"), New DataColumn("Temp_KirimSetuj"), New DataColumn("Temp_KirimActual"), New DataColumn("Temp_KirimTerima"), _
                                                New DataColumn("Temp_KirimTrmInp"), New DataColumn("Temp_KirimDlr"), New DataColumn("Temp_KirimTipe"), New DataColumn("Temp_KirimSales"), New DataColumn("Temp_KirimSPK"), _
                                                New DataColumn("Temp_KirimDO"), New DataColumn("Temp_KirimHP"), New DataColumn("Temp_KirimPHONE"), New DataColumn("Temp_KirimCust"), New DataColumn("Temp_KirimAlamat"), New DataColumn("Temp_KirimNote")})
        ViewState("BBM") = dt
        Me.BindGridBBM()
    End Sub
    ...
    Sub insertTabelBBM(ByVal Type As Byte, ByVal Temp_KirimRangka As String, ByVal Temp_KirimInput As String, ByVal Temp_KirimMohon As String, ByVal Temp_KirimSetuj As String, ByVal Temp_KirimActual As String, ByVal Temp_KirimTerima As String, ByVal Temp_KirimTrmInp As String, ByVal Temp_KirimDlr As String, ByVal Temp_KirimTipe As String, ByVal Temp_KirimSales As String, ByVal Temp_KirimSPK As String, ByVal Temp_KirimDO As String, ByVal Temp_KirimHP As String, ByVal Temp_KirimPHONE As String, ByVal Temp_KirimCust As String, ByVal Temp_KirimAlamat As String, ByVal Temp_KirimNote As String)
        Dim dt As DataTable = DirectCast(ViewState("BBM"), DataTable)
        If Type = 0 Then
            dt.Clear()
        Else
            dt.Rows.Add(Temp_KirimRangka, Temp_KirimInput, Temp_KirimMohon, Temp_KirimSetuj, Temp_KirimActual, Temp_KirimTerima, Temp_KirimTrmInp, Temp_KirimDlr, Temp_KirimTipe, Temp_KirimSales, Temp_KirimSPK, Temp_KirimDO, Temp_KirimHP, Temp_KirimPHONE, Temp_KirimCust, Temp_KirimAlamat, Temp_KirimNote)
        End If
        ViewState("BBM") = dt
        Me.BindGridBBM()
    End Sub
...
ErrHand:
    ...
    Sub KirimUnit(ByVal mQuery As String)
        ...
        Dim mSqlCommadstring As String = _
                     "SELECT STOCK_NoRangka,LOKASI_Nama,TYPE_Nama,STOCKM_TGLMOHONINPUT,STOCKM_TGLMOHONKIRIM,STOCKM_KIRIMTGLSETUJUI,STOCKK_KIRIMTGLKIRIM,STOCKK_KIRIMTGLTERIMA,STOCKK_KIRIMTGLTERIMAI,STOCKM_KDDEALER,STOCKM_SALES,STOCK_NOSPK,STOCK_DoDealer,STOCKA_ALAMAT,STOCKA_KOTA,STOCKA_HP,STOCKA_PH,STOCKA_NAMA,STOCKM_BERITA " & _
                     "FROM TRXN_STOCK,TRXN_STOCKAKIRIM,TRXN_STOCKMKIRIM,TRXN_STOCKKKIRIM,DATA_TYPE,DATA_LOKASI  WHERE " & _
                     "STOCK_Cdlokasi=lokasi_kode AND " & _
                     "STOCK_CdType=TYPE_Type AND " & _
                     "STOCK_NoRangka=STOCKA_NORANGKA AND " & _
                     "STOCK_NoRangka=STOCKM_NORANGKA AND " & _
                     "STOCK_NoRangka = STOCKK_NORANGKA AND " & mQuery
        insertTabelBBM(0, "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "")
        ...
        Try
            ...
            If MyRecReadA.HasRows = True Then
                While MyRecReadA.Read()
                    mLokasi = ""
                    insertTabelBBM(1, nSr(MyRecReadA("STOCK_NoRangka")), nSr(MyRecReadA("STOCKM_TGLMOHONINPUT")), nSr(MyRecReadA("STOCKM_TGLMOHONKIRIM")), nSr(MyRecReadA("STOCKK_KIRIMTGLKIRIM")), _
                                      nSr(MyRecReadA("STOCKM_KIRIMTGLSETUJUI")), nSr(MyRecReadA("STOCKK_KIRIMTGLTERIMA")), nSr(MyRecReadA("STOCKK_KIRIMTGLTERIMAI")), _
                                      nSr(MyRecReadA("STOCKM_KDDEALER")), nSr(MyRecReadA("TYPE_Nama")), nSr(MyRecReadA("STOCKM_SALES")), _
                                      nSr(MyRecReadA("STOCK_NOSPK")), nSr(MyRecReadA("STOCK_DoDealer")), nSr(MyRecReadA("LOKASI_NAMA")), _
                                      nSr(MyRecReadA("STOCKA_HP")) & "/" & nSr(MyRecReadA("STOCKA_PH")), nSr(MyRecReadA("STOCKA_NAMA")), nSr(MyRecReadA("STOCKA_ALAMAT")) & " " & nSr(MyRecReadA("STOCKA_KOTA")), nSr(MyRecReadA("STOCKM_BERITA")))
                End While
            End If
            MyRecReadA.Close()
            cmd.Dispose()
    ...
    Protected Sub Button2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button2.Click
        If txtTglStart.Text <> "" And txtTglEnd.Text <> "" Then
            ...
            Call KirimUnit("datediff(day," & DtInSQL(CDate(txtTglStart.Text)) & ",STOCKK_KIRIMTGLKIRIM) >= 0 and datediff(day,STOCKK_KIRIMTGLKIRIM," & DtInSQL(CDate(txtTglEnd.Text)) & ") >= 0")
        End If
    End Sub

    Protected Sub BtnCetak_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnCetak.Click
        Response.ClearContent()
        Response.AddHeader("content-disposition", "attachment; filename=KIRIMUNIT.xls")
