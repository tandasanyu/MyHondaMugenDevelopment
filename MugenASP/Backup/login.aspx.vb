...
ErrHand:
    ...
    Private Function ValidateUser(ByVal userName As String, ByVal passWord As String) As Boolean
        ...
        If Left(txtUserName.Text, 3) = "112" Or Left(txtUserName.Text, 3) = "128" Then
            If Len(txtUserName.Text) <= 3 Then
                userfound = 0
            Else
                userfound = BukaFile("SELECT * FROM DATA_SECURITYH,DATA_SECURITYU,DATA_SECURITYWA WHERE RTRIM(SECURITYH_USER)=RTRIM(USER_NAMA) AND RTRIM(USER_ACCESS)=RTRIM(AKSES_KODE) AND USER_NAMA= '" & Left(txtUserName.Text, 3) & "' AND SECURITYH_NOIDSALES='" & Right(txtUserName.Text, Len(txtUserName.Text) - 3) & "'", "SECURITYH_PASS", txtUserPass.Text)
            End If
        Else
            userfound = BukaFile("SELECT * FROM DATA_SECURITYU WHERE User_nama= '" & txtUserName.Text & "' AND User_Password= '" & txtUserPass.Text & "' AND User_tipe='WA'", "", "")
        End If
        ...
        ElseIf userfound = 2 Then   '2
            ...
            'End If
        Else                        '0
            LblError.Text = ("Nama Pemakai atau Kata kunci tidak diterima")
