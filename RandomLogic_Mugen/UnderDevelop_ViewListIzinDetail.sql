SELECT        Details.IZIN_ID, b.IZIN_NIK AS NIK, h.IZIN_NAMA, 
  ...
  END AS NIKMNG, h.IZIN_NIK_APPVSPV AS NIKSPV,
   (SELECT STAFF_NAMA FROM dbo.DATA_STAFF WHERE (STAFF_NIK = h.IZIN_NIK_APPVSPV)) AS Atasan1, 
      ...
      END AS Atasan2, 
	  --b.IZIN_TGLAPPVMNG, b.IZIN_TGLAPPVSPV, 

	  Details.id_detail, b.IZIN_JENIS, b.IZIN_ALASAN, 
      ...
      /*Panggil Approval MNG & SPV*/
	   Details.IZIN_APRRJT as APVSPV,Details.IZIN_APRRJTMNG as APVMNG,
      CASE
	  -- CASE EXPIRED
			...
			AND Details.IZIN_APRRJT IS NULL 
			...
			AND b.IZIN_JENIS = 'Cuti' AND b.IZIN_ALASAN = 'Cuti'
			AND (h.IZIN_NIK_APPVMNG <> '--' OR h.IZIN_NIK_APPVMNG IS NOT NULL)
			AND Details.IZIN_APRRJT IS NOT NULL AND Details.IZIN_APRRJTMNG IS NULL 
		...
		-- CASE APPROVE
			...
			AND b.IZIN_JENIS = 'Cuti' AND b.IZIN_ALASAN = 'Cuti'
			AND (h.IZIN_NIK_APPVMNG = '--' OR h.IZIN_NIK_APPVMNG IS NULL)
			AND Details.IZIN_APRRJT IS NOT NULL 
			...
			AND b.IZIN_JENIS = 'Cuti' AND b.IZIN_ALASAN = 'Cuti'
			AND (h.IZIN_NIK_APPVMNG <> '--' OR h.IZIN_NIK_APPVMNG IS NOT NULL)
			AND Details.IZIN_APRRJT IS NOT NULL AND Details.IZIN_APRRJTMNG IS NOT NULL
			AND(b.IZIN_STATUS = 'Disetujui Manajer' OR Details.izin_status = 'Disetujui Manajer')
			...
			AND b.IZIN_JENIS = 'Cuti' AND b.IZIN_ALASAN <> 'Cuti'
			--AND (h.IZIN_NIK_APPVMNG = '--' OR h.IZIN_NIK_APPVMNG IS NULL)
			AND Details.IZIN_APRRJT IS NOT NULL AND (B.IZIN_STATUS = 'Disetujui Manajer' OR B.IZIN_STATUS = 'Disetujui Atasan')
...
FROM dbo.DATA_IZIN_BODY AS b INNER JOIN 
 (SELECT IZIN_ID,IZIN_APRRJT, IZIN_APRRJTMNG , IZIN_NIK, IZIN_TGLDETAIL, IZIN_STATUS, IZIN_ID_DETAIL AS id_detail, 'DT' AS Source, Flag
    FROM dbo.DATA_IZIN_DETAIL
    UNION ALL
  SELECT IZIN_ID,IZIN_APRRJT, IZIN_APRRJTMNG , IZIN_NIK, IZIN_TGLDETAIL, IZIN_STATUS, IZIN_ID_DETAILPC AS id_detail, 'PC' AS Source, Flag
