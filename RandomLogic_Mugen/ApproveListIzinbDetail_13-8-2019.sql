/*
RULES : HANYA INPUT KE DETAIL KETIKA APPROVAL SAJA. SELAIN ITU DETAIL (STATUS = NULL)
*/
  ...
  END AS NIKMNG, h.IZIN_NIK_APPVSPV AS NIKSPV,
   (SELECT STAFF_NAMA FROM dbo.DATA_STAFF WHERE (STAFF_NIK = h.IZIN_NIK_APPVSPV)) AS Atasan1, 
      ...
      CASE
        ...
        --WHEN B.IZIN_STATUS = 'Disetujui Manajer' then 'Disetujui Manajer' -- Penambahan logic baru untuk covering izin yang telah di approve
        --WHEN B.IZIN_STATUS = 'Disetujui Atasan' THEN 'Disetujui Atasan'
        WHEN (SELECT izin_tgldeadline FROM data_izin_body WHERE izin_id = Details.izin_id) < CAST(CONVERT(CHAR(8), GETDATE(), 112) AS smalldatetime) /*AND Details.izin_status IS NULL*/ AND 
            b.IZIN_TGLAPPVSPV IS NULL AND b.IZIN_JENIS = 'Cuti'  AND h.IZIN_NIK_APPVMNG = '--' 
            THEN 'Expired Cuti 1 Atasan' 
        WHEN (SELECT izin_tgldeadline FROM data_izin_body WHERE izin_id = Details.izin_id) < CAST(CONVERT(CHAR(8), GETDATE(), 112) AS smalldatetime) /*AND Details.izin_status IS NULL*/ AND 
            b.IZIN_TGLAPPVSPV IS NULL AND b.IZIN_JENIS = 'Cuti' AND h.IZIN_NIK_APPVMNG <> '--' 
            THEN 'Expired Cuti 2 Atasan -- ATASAN 1 BLM APPRV' 
        WHEN (SELECT izin_tgldeadline FROM data_izin_body WHERE izin_id = Details.izin_id) < CAST(CONVERT(CHAR(8), GETDATE(), 112) AS smalldatetime) /*AND Details.izin_status IS NULL*/ AND 
            b.IZIN_TGLAPPVSPV IS NOT NULL AND b.IZIN_TGLAPPVMNG IS NULL AND b.IZIN_JENIS = 'Cuti' AND h.IZIN_NIK_APPVMNG <> '--' 
            THEN 'Expired Cuti 2 Atasan -- ATASAN 1 SDH APPV / ATASAN 2 BLM APPRV' 
        WHEN (SELECT izin_tgldeadline FROM  data_izin_body WHERE izin_id = Details.izin_id) < CAST(CONVERT(CHAR(8), GETDATE(), 112) AS smalldatetime) /*AND Details.izin_status IS NULL*/ AND 
            b.IZIN_TGLAPPVSPV IS NULL AND b.IZIN_JENIS <> 'Cuti' and b.IZIN_ALASAN <> 'Cuti'
            THEN 'Expired Selain Cuti' 
        WHEN Details.izin_status IS NOT NULL AND b.IZIN_STATUS <> 'Pending' AND h.IZIN_NIK_APPVMNG = '--' AND b.IZIN_TGLAPPVSPV IS NOT NULL AND b.IZIN_JENIS = 'Cuti' THEN 'Disetujui Atasan Langsung -- CUTI BIASA' 
        WHEN Details.izin_status IS NOT NULL AND b.IZIN_STATUS <> 'Pending' AND h.IZIN_NIK_APPVMNG <> '--' AND b.IZIN_TGLAPPVSPV IS NOT NULL AND b.IZIN_TGLAPPVMNG IS NOT NULL AND b.IZIN_JENIS = 'Cuti' THEN 'Disetujui Atasan 1tk Lebih Tinggi -- CUTI BIASA' 
        WHEN Details.izin_status IS NOT NULL AND b.IZIN_STATUS <> 'Pending' AND h.IZIN_NIK_APPVMNG <> '--' AND b.IZIN_TGLAPPVSPV IS NOT NULL AND b.IZIN_TGLAPPVMNG IS NULL AND b.IZIN_JENIS = 'Cuti' THEN 'Disetujui Atasan Langsung -- CUTI BIASA / DELAY' 
        WHEN Details.izin_status IS NOT NULL AND b.IZIN_STATUS <> 'Pending' AND b.IZIN_TGLAPPVSPV IS NOT NULL AND b.IZIN_JENIS <> 'Cuti' THEN 'Disetujui Atasan Langsung -- SELAIN CUTI BIASA'        
        ELSE NULL
      END AS Status_Izin, Details.Source, Details.Flag
