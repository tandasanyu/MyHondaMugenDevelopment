   ...
   (SELECT STAFF_NAMA FROM dbo.DATA_STAFF WHERE (STAFF_NIK = h.IZIN_NIK_APPVSPV)) AS Atasan1, 
      ...
      CASE 
        WHEN (SELECT izin_tgldeadline FROM data_izin_body WHERE izin_id = Details.izin_id) >= CAST(CONVERT(CHAR(8), GETDATE(), 112) AS smalldatetime) AND Details.izin_status IS NULL AND 
            b.IZIN_STATUS = 'Pending' THEN 'Pending' 
        WHEN b.IZIN_STATUS = 'Dibatalkan Staff' THEN 'Dibatalkan Staff' 
        WHEN b.IZIN_STATUS = 'Dibatalkan HRD' THEN 'Dibatalkan HRD' 
        WHEN (SELECT izin_tgldeadline FROM data_izin_body WHERE izin_id = Details.izin_id) < CAST(CONVERT(CHAR(8), GETDATE(), 112) AS smalldatetime) AND Details.izin_status IS NULL AND 
            b.IZIN_TGLAPPVSPV IS NULL AND b.IZIN_JENIS = 'Cuti' AND h.IZIN_NIK_APPVMNG = '--' THEN 'Expired Cuti 1 Atasan' 
        WHEN (SELECT izin_tgldeadline FROM data_izin_body WHERE izin_id = Details.izin_id) < CAST(CONVERT(CHAR(8), GETDATE(), 112) AS smalldatetime) AND Details.izin_status IS NULL AND 
            b.IZIN_TGLAPPVSPV IS NULL AND b.IZIN_JENIS = 'Cuti' AND h.IZIN_NIK_APPVMNG <> '--' THEN 'Expired Cuti 2 Atasan -- ATASAN 1 BLM APPRV' 
        WHEN (SELECT izin_tgldeadline FROM data_izin_body WHERE izin_id = Details.izin_id) < CAST(CONVERT(CHAR(8), GETDATE(), 112) AS smalldatetime) AND Details.izin_status IS NULL AND 
            b.IZIN_TGLAPPVSPV IS NOT NULL AND b.IZIN_TGLAPPVMNG IS NULL AND b.IZIN_JENIS = 'Cuti' AND h.IZIN_NIK_APPVMNG <> '--' THEN 'Expired Cuti 2 Atasan -- ATASAN 1 SDH APPV / ATASAN 2 BLM APPRV' 
        WHEN (SELECT izin_tgldeadline FROM  data_izin_body WHERE izin_id = Details.izin_id) < CAST(CONVERT(CHAR(8), GETDATE(), 112) AS smalldatetime) AND Details.izin_status IS NULL AND 
            b.IZIN_TGLAPPVSPV IS NULL AND b.IZIN_JENIS <> 'Cuti' THEN 'Expired Selain Cuti' 
