<%@ Page Language="C#" AutoEventWireup="true" CodeFile="jobcontrolformanclopuri.aspx.cs" Inherits="jobcontrolformanclopuri" %>
      ...
      <div id="hakAksesclosing" runat="server">
        ...
        <table style="font-size:1.0em;width:100%;"><tr><td><center style="background:#c0c0c0;">Data Proses</center>
         <asp:DataList ID="DataList2" runat="server" DataSourceID="SqlDataSource5" style="width:100%;float:left;">
             ...
             <asp:SqlDataSource ID="SqlDataSource5" runat="server" ConnectionString="<%$ ConnectionStrings:service128Connection %>" SelectCommand="SELECT WOHDR_TGWO, WOHDR_FNOPOL, WOHDR_FNMS, WOHDR_FNRK, WOHDR_FORG, WOHDR_KDTAGIH, WOHDR_SA, WOHDR_NO FROM TRXN_WOHDR WHERE (WOHDR_NO = @WOHDR_FNOPOL2)">
                 <SelectParameters>
...
<div id="show" style="border: 1px solid white; display: none; margin: 5px; padding: 2px; width: 98%;">
    <asp:Image ID="Image2" runat="server"  Style="width:600px;height:400px;"/>
      ...
      <asp:GridView ID="GridView5" runat="server" AutoGenerateColumns="False" DataSourceID="SqlDataSource7" EnableModelValidation="True" class="table table-striped" AllowSorting="True" CellPadding="4" ForeColor="#333333" GridLines="None">
         ...
         <asp:SqlDataSource ID="SqlDataSource7" runat="server" ConnectionString="<%$ ConnectionStrings:service128Connection %>" SelectCommand="SELECT [KERJABODY_NOWO], [KERJABODY_TANGGAL], [KERJABODY_USER], case [KERJABODY_STATUS] when 1 then 'DITERIMA' when 2 then 'BONGKAR' when 3 then 'KETOK' when 4 then 'DEMPUL' when 5 then 'CAT/OVEN' when 6 then 'POLES' when 7 then 'PEMASANGAN' when 8 then 'FINISHING' when 0 then 'CLOSING' when 9 then 'ANTRIAN' else 'UNCATEGORIZED' end AS statusval, case [KERJABODY_LOKASI] when 1 then 'lt. 1' when 2 then 'lt. 2' when 3 then 'lt. 3' when 4 then 'lt.4' when 5 then 'lt. 5' when 6 then 'lt. 6' when 7 then 'lt. 7' when 8 then 'lt. 8' when 9 then 'lt. 9' else '' END AS lokasimobil, [KERJABODY_CATATAN] FROM [TEMP_KERJABODY] WHERE ([KERJABODY_NOWO] = @KERJABODY_NOWO2) ORDER BY KERJABODY_STATUS ASC">
             <SelectParameters>
