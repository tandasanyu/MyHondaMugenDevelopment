...
<asp:UpdatePanel ID="UpdatePanel1" runat="server">
     <ContentTemplate>
         ...
         <asp:GridView ID="GridView6" runat="server" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False" CellPadding="4" DataSourceID="SqlDataSource8" EnableModelValidation="True" ForeColor="#333333" GridLines="None" class="table table-bordered table-stripped" style="width:98%;margin:5px;font-size:0.85em;font-family:verdana;color:#333;">
             ...
             <asp:SqlDataSource ID="SqlDataSource8" runat="server" ConnectionString="<%$ ConnectionStrings:serviceConnection %>" SelectCommand="SELECT WOHDR_TGWO, WOHDR_FNOPOL, WOHDR_FNMS, WOHDR_FNRK, WOHDR_FORG, WOHDR_KDTAGIH, WOHDR_SA, WOHDR_NO, CONTROLBR_TGLESELESAI, case [CONTROLBR_KETOKNILAI] when 1 then 'DITERIMA' when 2 then 'BONGKAR' when 3 then 'KETOK' when 4 then 'DEMPUL' when 5 then 'CAT/OVEN' when 6 then 'POLES' when 7 then 'PEMASANGAN' when 8 then 'FINISHING' when 0 then 'CLOSING' when 9 then 'ANTRIAN' else '' end AS statuskerja, SUPPLIER_NAMA FROM TRXN_WOHDR INNER JOIN TEMP_CONTROLBR ON TRXN_WOHDR.WOHDR_NO = TEMP_CONTROLBR.CONTROLBR_NOWO INNER JOIN DATA_SUPPLIER ON TRXN_WOHDR.WOHDR_KDTAGIH = DATA_SUPPLIER.SUPPLIER_KODE WHERE (WOHDR_NO = @cari) OR (WOHDR_FNOPOL LIKE '%' + @cari + '%')">
                  <SelectParameters>
            ...
            <asp:GridView ID="GridView2" runat="server" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False" CellPadding="4" DataSourceID="SqlDataSource3" EnableModelValidation="True" ForeColor="#333333" GridLines="None" class="table table-bordered table-stripped" style="width:98%;margin:5px;font-size:0.85em;font-family:verdana;color:#333;">
             ...
             <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:serviceConnection %>" SelectCommand="SELECT WOHDR_TGWO, WOHDR_FNOPOL, WOHDR_FNMS, WOHDR_FNRK, WOHDR_FORG, WOHDR_KDTAGIH, WOHDR_SA, WOHDR_NO, CONTROLBR_TGLESELESAI, case [CONTROLBR_KETOKNILAI] when 1 then 'DITERIMA' when 2 then 'BONGKAR' when 3 then 'KETOK' when 4 then 'DEMPUL' when 5 then 'CAT/OVEN' when 6 then 'POLES' when 7 then 'PEMASANGAN' when 8 then 'FINISHING' WHEN 0 then 'Closing' when 9 then 'ANTRIAN' else '' end AS statuskerja, SUPPLIER_NAMA, (select max(KERJABODY_TANGGAL)  from TEMP_KERJABODY WHERE KERJABODY_NOWO =WOHDR_NO GROUP BY KERJABODY_NOWO) AS tglAkhir FROM TRXN_WOHDR , TEMP_CONTROLBR,DATA_SUPPLIER where WOHDR_NO = CONTROLBR_NOWO and WOHDR_KDTAGIH = SUPPLIER_KODE and CONTROLBR_TGLSELESAIA IS NULL"></asp:SqlDataSource>
     </ContentTemplate>
</asp:UpdatePanel>
