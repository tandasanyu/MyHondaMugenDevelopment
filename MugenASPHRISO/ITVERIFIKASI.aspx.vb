...
Partial Class ITVERIFIKASI
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If LblUserName.Text = "" Then
            ...
            If GetData_UserSecurity("SELECT * FROM tb_userutility WHERE username = '" & LblUserName.Text & "' And hakakses = 'FORM VERIFIKASI -- VIEW DATA'") = 1 Then
                MultiViewAkses.ActiveViewIndex = 0
                If GetData_UserSecurity("SELECT * FROM tb_userutility WHERE username = '" & LblUserName.Text & "' and hakakses = 'FORM VERIFIKASI -- ADD DATA'") = 1 Then
    ...
    Protected Sub BtnMasterTabel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnMasterTabel.Click
        If GetData_UserSecurity("SELECT * FROM tb_userutility WHERE username = '" & LblUserName.Text & "' and hakakses = 'FORM VERIFIKASI -- ADD DATA'") = 1 Then
            ...
            If lblVerifikasiId.Text = "" Then
                lblVerifikasiId.Text = GetData_SearchNomor()
            End If
        Else
    ...
    Protected Sub BtnNilaiSMSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnNilaiSMSave.Click
        If lblVerifikasiId.Text = "" Then
            lblVerifikasiId.Text = lblVerifikasiId.Text
            UpdateData_Server(insertData)
        Else
            If GetFindRecord_Server("SELECT * FROM TRXN_VERIFIKASI WHERE VERIFIKASI_NO='" & lblVerifikasiId.Text & "'") <> 1 Then
            ...
            Response.Write("<script>alert('Jadwal Verifikasi Program Berhasil Dibuat')</script>")
            Response.Write("<script>window.location.href='ITVERIFIKASI.aspx';</script>")
        End If
    End Sub
    ...
    Protected Sub LvDetailMaster_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles LvDetailMaster.SelectedIndexChanged
        If GetData_UserSecurity("SELECT * FROM tb_userutility WHERE username = '" & LblUserName.Text & "' and hakakses = 'FORM VERIFIKASI -- CLOSE DATA'") = 1 Then
            lblVerifikasiId.Text = (LvDetailMaster.DataKeys(LvDetailMaster.SelectedIndex).Value.ToString)
            If CountData_Master("SELECT * FROM TRXN_VERIFIKASI WHERE VERIFIKASI_NO='" & lblVerifikasiId.Text & "' AND VERIFIKASI_REALISASI IS NOT NULL") = 1 Then
                Response.Write("<script>alert('Jadwal Verifikasi Program Sudah di Close!')</script>")
            Else
                TxtHasil.Text = ""
                TxtKeterangan.Text = ""
                Call GetData_AddNomor("SELECT * FROM TRXN_VERIFIKASI WHERE VERIFIKASI_NO='" & lblVerifikasiId.Text & "'")
                Call GetData_Validasi("SELECT * FROM divisi WHERE nmdivisi = '" & lblDepartemen.Text & "'")
    ...
    Protected Sub BtnNilaiSMSave2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnNilaiSMSave2.Click
        ...
        LvDetailMaster.DataBind()
        Call emailnotifawal_server("select emaildivisi from divisi where nmdivisi='" & lblDepartemen.Text & "'")
        Response.Write("<script>alert('Jadwal Verifikasi Program Berhasil Di Close')</script>")
        Response.Write("<script>window.location.href='ITVERIFIKASI.aspx';</script>")
    End Sub
    ...
    Function insertData() As String
        insertData = "INSERT INTO TRXN_VERIFIKASI (VERIFIKASI_NO) VALUES ('" & lblVerifikasiId.Text & "')"
    End Function

    Function EDITData() As String
        EDITData = "UPDATE TRXN_VERIFIKASI SET " & _
                                "VERIFIKASI_JADWAL='" & TxtJadwal.Text & "'," & _
                                "VERIFIKASI_DEPARTEMEN='" & DropDownList1.Text & "'," & _
                                "VERIFIKASI_DIBUATOLEH='" & LblUserName.Text & "'," & _
                                "VERIFIKASI_TGLDIBUAT='" & Now() & "'," & _
                                "VERIFIKASI_PROGRAM='" & TxtProgram.Text & "'" & _
                                "WHERE VERIFIKASI_NO='" & lblVerifikasiId.Text & "'"
    End Function

    Function EDITHasil() As String
        EDITHasil = "UPDATE TRXN_VERIFIKASI SET " & _
                        "VERIFIKASI_REALISASI='" & Now() & "'," & _
                        "VERIFIKASI_HASIL='" & TxtHasil.Text & "'," & _
                        "VERIFIKASI_VALIDASIUSER2='" & Labelhead.Text & "'," & _
                        "VERIFIKASI_KETERANGAN='" & TxtKeterangan.Text & "'" & _
                        "WHERE VERIFIKASI_NO='" & lblID.Text & "'"
    End Function
    Function GetData_AddNomor(ByVal mSqlCommadstring As String) As Double
        ...
        Try
            ...
            If MyRecReadA.HasRows = True Then
                MyRecReadA.Read()
                lblID.Text = nSr(MyRecReadA("VERIFIKASI_NO"))
                lblDepartemen.Text = nSr(MyRecReadA("VERIFIKASI_DEPARTEMEN"))
                lblJadwal.Text = nSr(MyRecReadA("VERIFIKASI_JADWAL"))
                lblProgram.Text = nSr(MyRecReadA("VERIFIKASI_PROGRAM"))
    ...
    Function GetData_SearchNomor() As String
        ...
        mSqlCommadstring = "SELECT COUNT(VERIFIKASI_TGLDIBUAT) as maxPemohon from trxn_VERIFIKASI where DATEPART(month, VERIFIKASI_TGLDIBUAT) = '" + mBulan + "' AND DATEPART(year, VERIFIKASI_TGLDIBUAT) = '" + mTahun + "'"
        cnn = New OleDbConnection(strconn)
        Try
            cnn.Open()
            cmd = New OleDbCommand(mSqlCommadstring, cnn)
    ...
    Function GetData_Master(ByVal mSqlCommadstring As String) As Double
        ...
        Try
            ...
            If MyRecReadA.HasRows = True Then
                MyRecReadA.Read()
                lblID.Text = nSr(MyRecReadA("VERIFIKASI_NO"))
                lblDepartemen.Text = nSr(MyRecReadA("VERIFIKASI_DEPT"))
                lblJadwal.Text = nSr(MyRecReadA("VERIFIKASI_JADWAL"))
                lblProgram.Text = nSr(MyRecReadA("VERIFIKASI_PROGRAM"))
            End If
            MyRecReadA.Close()
    ...
    Function GetData_Validasi(ByVal mSqlCommadstring As String) As Byte
        ...
        Try
            ...
            While MyRecReadA.Read()
                Labelhead.Text = MyRecReadA("headdivisi")
            End While
            cmd.Dispose()
            cnn.Close()
        Catch ex As Exception
