...
Partial Class HRDPENILAIANKARYAWANC
    ...
    Protected Sub LvProses2_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles LvProses2.SelectedIndexChanged
        ...
        If LblProsesKodeItem.Text = "PRC" Then
            ...
            ElseIf LblUserName.Text = TxtHead2.Text Then
                ...
                Call GetData_NilaiBawahan("select avg(cast(kpin_nilai as int)*10) as nilai_bawahan from trxn_kpin where (KPIN_ATASAN = '" & TXTUsername.Text & "' Or KPIN_ATASAN = '" & TxtStaffNama.Text & "') and kpin_kode <> '017'")
                Call inputNilaiProses()
            ElseIf LblUserName.Text = "HRD112" Then
                Call GetData_MASTERKPIProses2("SELECT * FROM TRXN_KPIDD,DATA_KPIN WHERE KPI_KODEITEM=KPIN_TIPE AND KPI_NIK='" & TxtStaffNIK.Text & "' AND KPI_TAHUN='" & TxtKpiSummaryTahun.Text & "' AND KPI_KODEITEM ='" & TxtPetik(LblProsesKodeItem2.Text) & "'")
                Call GetData_NilaiBawahan("select avg(cast(kpin_nilai as int)*10) as nilai_bawahan from trxn_kpin where (KPIN_ATASAN = '" & TXTUsername.Text & "' Or KPIN_ATASAN = '" & TxtStaffNama.Text & "') and kpin_kode <> '017'")
                Call inputNilaiProses()
            Else
    ...
    Protected Sub BtnKpi44Save_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnKpi44Setuju1.Click
        If GetFindRecord_Server("select kpi_staff, KPI_KODEITEMM, KPIN_JUDUL from trxn_kpidd where (kpi_staff is null or kpi_staff = 0) and kpi_kodeitem like 'B%' and kpi_nik='" & TxtStaffNIK.Text & "' and kpi_tahun='" & TxtKpiSummaryTahun.Text & "' and KPIN_TIPE=KPI_KODEITEMM") = 1 Then
            ...
            MultiViewKpiMasterEntry.ActiveViewIndex = 0
            Call ClikDetail_MultiViewKpiMaster()
        ElseIf GetFindRecord_Server("select kpi_staff, KPI_KODEITEMM, KPIN_JUDUL from trxn_kpidd where (kpi_staff = 0 or kpi_staff is null)  and kpi_kodeitem like 'M%' and kpi_nik='" & TxtStaffNIK.Text & "' and kpi_tahun='" & TxtKpiSummaryTahun.Text & "' and KPIN_TIPE=KPI_KODEITEMM") = 1 Then
    ...
    Protected Sub BtnKpi44Setuju2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnKpi44Setuju2.Click
        If GetFindRecord_Server2("select kpi_atasan, KPI_KODEITEMM, KPIN_JUDUL from trxn_kpidd, DATA_KPIN where (kpi_atasan is null or kpi_atasan = 0) and kpi_kodeitem like 'B%' and kpi_nik='" & TxtStaffNIK.Text & "' and kpi_tahun='" & TxtKpiSummaryTahun.Text & "' and KPIN_TIPE=KPI_KODEITEMM") = 1 Then
            Response.Write("<script>alert('Anda Penilaian Item Proses Yang Belum Anda Isi dibagian " + TxtNotif.Text + ", Silahkan di cek kembali!')</script>")
            MultiViewKpiMasterEntry.ActiveViewIndex = 0
            Call ClikDetail_MultiViewKpiMaster()
        ElseIf GetFindRecord_Server2("select kpi_atasan, KPI_KODEITEMM, KPIN_JUDUL from trxn_kpidd where (kpi_atasan = 0 or kpi_atasan is null)  and kpi_kodeitem like 'M%' and kpi_nik='" & TxtStaffNIK.Text & "' and kpi_tahun='" & TxtKpiSummaryTahun.Text & "' and KPIN_TIPE=KPI_KODEITEMM") = 1 Then
            Response.Write("<script>alert('Anda Penilaian Item People Management Yang Belum Anda Isi dibagian " + TxtNotif.Text + ", Silahkan di cek kembali!')</script>")
            MultiViewKpiMasterEntry.ActiveViewIndex = 0
            Call ClikDetail_MultiViewKpiMaster()
        Else
