...
Partial Class ISOINSTRUKSIKERJA
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If LblUserName.Text = "" Then
            ...
            If GetData_UserSecurity("SELECT * FROM tb_user inner join tb_userutility on tb_user.username collate SQL_Latin1_General_CP1_CI_AS = tb_userutility.username collate SQL_Latin1_General_CP1_CI_AS WHERE tb_user.username = '" & LblUserName.Text & "' And tb_userutility.hakakses = 'FORM INSTRUKSI KERJA -- ADD+DISTRIBUSI DATA'") = 1 Then
                MultiViewDept.ActiveViewIndex = 0
                LvDetailMaster.DataBind()
                Call ActiveButton()
            ElseIf GetData_UserSecurity("SELECT * FROM tb_user inner join tb_userutility on tb_user.username collate SQL_Latin1_General_CP1_CI_AS = tb_userutility.username collate SQL_Latin1_General_CP1_CI_AS WHERE tb_user.username = '" & LblUserName.Text & "' And tb_userutility.hakakses = 'FORM INSTRUKSI KERJA -- ADD DATA'") = 1 Then
                MultiViewInstruksiKerja.ActiveViewIndex = 0
                LvDetailView.DataBind()
                Call ActiveButton2()
            ElseIf GetData_UserSecurity("SELECT * FROM tb_user inner join tb_userutility on tb_user.username collate SQL_Latin1_General_CP1_CI_AS = tb_userutility.username collate SQL_Latin1_General_CP1_CI_AS WHERE tb_user.username = '" & LblUserName.Text & "' And tb_userutility.hakakses = 'FORM INSTRUKSI KERJA -- VIEW DATA'") = 1 Then
                MultiViewInstruksiKerja.ActiveViewIndex = 0
                Call DisableButton()
            Else
                ...
                MultiViewDept.ActiveViewIndex = -1
                MultiViewObsolete.ActiveViewIndex = -1
                MultiViewEditInstruksiKerja.ActiveViewIndex = -1
                MultiViewFormEditDokumen.ActiveViewIndex = -1
                MultiViewInstruksiKerja.ActiveViewIndex = -1
                Call DisableButton()
    ...
    Protected Sub BtnMasterTabel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnMasterTabel.Click
        If GetData_UserSecurity("SELECT * FROM tb_userutility WHERE username = '" & LblUserName.Text & "' and hakakses = 'FORM INSTRUKSI KERJA -- ADD DATA' OR hakakses = 'FORM INSTRUKSI KERJA -- ADD+DISTRIBUSI DATA'") = 1 Then
            LvDetailMaster.DataBind()
    ...
    Protected Sub BtnNilaiSMSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnNilaiSMSave.Click
        If GetFindRecord_Server("SELECT * FROM TRXN_DOKUMEN WHERE DOKUMEN_NO='" & lblBackupId.Text & "'") <> 1 Then
            ...
            If status.Text = " Baru" Then
                ...
                Call UpdateData_Server("INSERT INTO TRXN_DOKUMEN (DOKUMEN_NO,DOKUMEN_NAMA,DOKUMEN_LINK,DOKUMEN_JENIS, DOKUMEN_DIBUAT, DOKUMEN_DIBUAT2, DOKUMEN_KETERANGAN, DOKUMEN_TGLUPLOAD, DOKUMEN_DEPARTMENT) VALUES ('" & lblBackupId.Text & "','" & TxtNamaDokumen.Text & "','" & filename & "', 'C', '" & dibuat.Text & "','" & dibuat2.Text & "','" & txtKeterangan.Text & "', '" & Now() & "', '" & department.Text & "')")
                Dim fileuploadpath As String = Server.MapPath("~/WEBDOWNLOAD/DOKUMENISO/")
                FileUpload1.PostedFile.SaveAs(Path.Combine(fileuploadpath, filename))
                Call KirimPesan_server("select * from tb_user where jabatan= '" & dibuat.Text & "'")
                If dibuat.Text <> "" Then
                    Call KirimPesan_server("select * from tb_user where jabatan= '" & dibuat2.Text & "'")
                End If
                LvDetailMaster.DataBind()
                Response.Write("<script>alert('Dokumen Instruksi Kerja berhasil ditambah!')</script>")
                Response.Write("<script>window.location.href='ISOINSTRUKSIKERJA.aspx';</script>")
            ElseIf status.Text = " Obsolete Dokumen" Then
                Dim filereceived As String = FileUpload1.PostedFile.FileName
                Dim filename As String = Path.GetFileName(filereceived)
                Call UpdateData_Server("INSERT INTO TRXN_DOKUMENO (DOKUMENO_NAMA,DOKUMENO_TGLTIDAKBERLAKU,DOKUMENO_LINK,DOKUMENO_DIBUAT,DOKUMENO_JENIS,DOKUMENO_KETERANGAN) VALUES ('" & TxtNamaDokumen.Text & "','" & Now() & "','" & filename & "', '" & LblUserName.Text & "','C','" & txtKeterangan.Text & "')")
                Dim fileuploadpath As String = Server.MapPath("~/WEBDOWNLOAD/DOKUMENISO/")
                FileUpload1.PostedFile.SaveAs(Path.Combine(fileuploadpath, filename))
                ListObsolete.DataBind()
                Response.Write("<script>alert('Dokumen Obsolete Instruksi Kerja berhasil ditambah!')</script>")
                MultiViewUploadDokumen.ActiveViewIndex = -1
                MultiViewAkses.ActiveViewIndex = -1
                MultiViewDistribusiDokumen.ActiveViewIndex = -1
    ...
    Protected Sub BtnNilaiSMSave3_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnNilaiSM3Save.Click
        If GetFindRecord_Server("SELECT * FROM TRXN_DOKUMEN WHERE DOKUMEN_NO='" & lblBackupId.Text & "'") = 1 Then
            ...
            Call UpdateData_Server("UPDATE TRXN_DOKUMEN SET " & _
              "DOKUMEN_TGLDIBUAT=NULL," & _
               "DOKUMEN_DIKETAHUI=''," & _
               "DOKUMEN_TGLDIKETAHUI=NULL," & _
               "DOKUMEN_DISETUJUI=''," & _
               "DOKUMEN_TGLDISETUJUI=NULL," & _
            ...
            'Call KirimPesan_server() : Exit Sub ****memang di nonaktifkan
        End If
        LvDetailMaster.DataBind()
        Response.Write("<script>alert('Dokumen Instruksi Kerja berhasil diperbarui!')</script>")
        MultiViewUploadDokumen.ActiveViewIndex = -1
        MultiViewAkses.ActiveViewIndex = -1
        MultiViewDistribusiDokumen.ActiveViewIndex = -1
        MultiViewEditInstruksiKerja.ActiveViewIndex = 0
        MultiViewFormEditDokumen.ActiveViewIndex = -1
        MultiViewObsolete.ActiveViewIndex = -1
    End Sub

    'Event simpan ketika melakukan distribusi terhadap file instruksi kerja yang sudah diapprove direksi
    Protected Sub BtnNilaiSM2Save_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnNilaiSM2Save.Click
        If GetFindRecord_Server("SELECT * FROM TRXN_DOKUMEND WHERE DOKUMEND_NO='" & lblBackupId.Text & "'") = 1 Then
            ...
            End If
            If D25.Checked Then
                Call UpdateData_Server("INSERT INTO TRXN_DOKUMEND (DOKUMEND_NO, DOKUMEND_DISTRIBUSI) VALUES ('" & lblBackupId.Text & "', 'D25')")
                Call emaildistribusi_server("select * from tb_user where jabatan= 'Marketing & Delivery Director'")
            End If
        ElseIf GetFindRecord_Server("SELECT * FROM TRXN_DOKUMEND WHERE DOKUMEND_NO='" & lblBackupId.Text & "'") <> 1 Then
            ...
            If D24.Checked Then
                ...
                Call emaildistribusi_server("select * from tb_user where jabatan= 'Purchasing'")
            End If
            If D25.Checked Then 'Marketing & Delivery Director
                Call UpdateData_Server("INSERT INTO TRXN_DOKUMEND (DOKUMEND_NO, DOKUMEND_DISTRIBUSI) VALUES ('" & lblBackupId.Text & "', 'D25')")
                Call emaildistribusi_server("select * from tb_user where jabatan= 'Marketing & Delivery Director'")
            End If
        End If
        LvDetailMaster.DataBind()
        Response.Write("<script>alert('Dokumen Instruksi Kerja berhasil didistribusikan!')</script>")
        Response.Write("<script>window.location.href='ISOINSTRUKSIKERJA.aspx';</script>")
    End Sub
    ...
    Protected Sub LvDetailMaster_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles LvDetailMaster.SelectedIndexChanged
        If GetData_UserSecurity("SELECT * FROM tb_userutility WHERE username = '" & LblUserName.Text & "' and hakakses = 'FORM INSTRUKSI KERJA -- ADD+DISTRIBUSI DATA'") = 1 Then
            ...
            If CountData_Master("SELECT * FROM TRXN_DOKUMEN WHERE DOKUMEN_NO='" & lblBackupId.Text & "'  AND DOKUMEN_DISETUJUI IS NULL OR DOKUMEN_DISETUJUI=''") = 1 Then
                ...
                Response.Write("<script>alert('Dokumen Instruksi Kerja Belum Disetujui!')</script>") : Exit Sub
            Else
    ...
    'Event ketika menekan tombol distribusi dokumen
    Protected Sub LvDetailDept_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles LvDetailDept.SelectedIndexChanged
        lblBackupId.Text = (LvDetailDept.DataKeys(LvDetailDept.SelectedIndex).Value.ToString)
        If GetData_UserSecurity("SELECT * FROM tb_user inner join tb_userutility on tb_user.username collate SQL_Latin1_General_CP1_CI_AS = tb_userutility.username collate SQL_Latin1_General_CP1_CI_AS WHERE tb_user.username = '" & LblUserName.Text & "' And tb_userutility.hakakses = 'FORM SOP -- ADD+DISTRIBUSI DATA'") = 1 Then
            lblArea2.Text = lblBackupId.Text
    ...
    Protected Sub BtnMasterInstruksi_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnMasterInstruksi.Click
        If GetData_UserSecurity("SELECT * FROM tb_user inner join tb_userutility on tb_user.username collate SQL_Latin1_General_CP1_CI_AS = tb_userutility.username collate SQL_Latin1_General_CP1_CI_AS WHERE tb_user.username = '" & LblUserName.Text & "' And tb_userutility.hakakses = 'FORM INSTRUKSI KERJA -- ADD+DISTRIBUSI DATA'") = 1 Then
            MultiViewUploadDokumen.ActiveViewIndex = -1
            MultiViewAkses.ActiveViewIndex = -1
            MultiViewDept.ActiveViewIndex = 0
    ...
    Function GetData_SearchNomor() As String
        ...
        mSqlCommadstring = "SELECT COUNT(DOKUMEN_TGLUPLOAD) as maxPemohon from trxn_DOKUMEN where DATEPART(month, DOKUMEN_TGLUPLOAD) = '" + mBulan + "' AND DATEPART(year, DOKUMEN_TGLUPLOAD) = '" + mTahun + "'"
        cnn = New OleDbConnection(strconn)
        Try
            cnn.Open()
            cmd = New OleDbCommand(mSqlCommadstring, cnn)
            ...
            Else
                ...
                Else
                    ...
                    GetData_SearchNomor = "IK" & mTahun & mBulan & GetData_SearchNomor
                End If
            End If
            MyRecReadA.Close()
            cmd.Dispose()
        ...
        Try
            ...
            While MyRecReadA.Read()
                ...
                If nSr(MyRecReadA("DOKUMEND_DISTRIBUSI")) = "D24" Then
                    D24.Checked = True
                End If
                If nSr(MyRecReadA("DOKUMEND_DISTRIBUSI")) = "D25" Then
                    D25.Checked = True
                End If
            End While
            MyRecReadA.Close()
        ...
        Try
            ...
            While MyRecReadA.Read()
                ...
                ElseIf nSr(MyRecReadA("JABATAN")) = "Purchasing" Then
                    lblArea1.Text = "D24"
                ElseIf nSr(MyRecReadA("JABATAN")) = "Marketing & Delivery Director" Then
                    lblArea1.Text = "D25"
                End If
            End While
            MyRecReadA.Close()
...
ErrHand:
    ...
    Sub DisableButton()
        ...
        BtnMasterInstruksi.Visible = False
    ...
    Sub ActiveButton()
        ...
        BtnMasterInstruksi.Visible = True
    ...
    Function KirimPesan_server(ByVal mSqlCommadstring As String) As Byte
        Dim strconn As String = WebConfigurationManager.ConnectionStrings("MyConnCloudDnet").ConnectionString
    ...
    Function emaildistribusi_server(ByVal mSqlCommadstring As String) As Byte
        ...
        Try
            ...
            While MyRecReadA.Read()
                Dim email As String = nSr(MyRecReadA("email"))
                Dim pesan As New MailMessage
                pesan.Subject = "Terdapat Distribusi Dokumen Instruksi Kerja Untuk Anda"
                pesan.To.Add(email) 'email tujuan
                pesan.From = New MailAddress("hmugen1991@gmail.com") 'email kalian
                pesan.Body = "<div style='background:linear-gradient(#c0c0c0, #f5f5f5);border-bottom:1px solid #d60000;padding:5px;font-family:verdana;'>Pemberitahuan Honda Mugen Web System</div><div style='font-family:verdana;font-size:10pt;padding:4px;margin:5px auto;border-left:3px solid #0080c0;'><div style='margin:0 auto;background: #FFfFD5;width:50%;padding:5px;border-radius:4px;border-left:3px solid #0080c0;font-family:verdana;font-size:10pt;'>Anda mendapat Distribusi Dokumen Instruksi Kerja, silahkan di cek <br/><span style='color:blue;font-size:8pt;'></span></div><br/><center><a href='http://office.hondamugen.co.id:8082/login.aspx' style='padding:6px;border:1px solid #f5f5f5;background:#0080c0;color:#f5f5f5;text-decoration:none;' target='_blank'>Klik untuk Lihat Dokumen</a></center><br><center style='color: blue; '>**Pastikan perangkat anda terhubung dengan WI-FI/LAN yang ada di Honda Mugen.</center><br/><center>Atas perhatian dan kerjasamanya kami ucapkan terimakasih.</center><br/><br/><br/><strong style='color:blue;font-family:verdana;'>Regards,<br/><i style='font-size:8pt;color:#666;'>Web Mugen System | Developed by IT Department</i></strong><br/><strong>Honda Mugen Group</strong><br/><i>PT. Mitrausaha Gentaniaga</i><br/><span style='font-size:9pt;color:#666;'>Jl.Raya Pasar Minggu No.10, Jakarta 12740 - Indonesia<br/>Jl.Lingkar luar barat, Cengkareng, Jakarta Barat - Indonesia</span><br/>Telp : (021) 797 3000(Show Room), 797 2000 (Bengkel)<br/>Fax &nbsp;: (021) 797 3834, 798 4735<br/>Website : <a href='www.hondamugen.co.id' target='_blank'>www.hondamugen.co.id</a><br/></div>"
                pesan.IsBodyHtml = True
                Dim SMTP As New SmtpClient("smtp.gmail.com")
                SMTP.EnableSsl = True
                SMTP.Credentials = New System.Net.NetworkCredential("hmugen1991@gmail.com", "112m128p") 'isi dengan info akun gmail kalian
