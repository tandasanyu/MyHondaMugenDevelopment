...
Partial Class HRDPENILAIANKARYAWAN
    ...
    Sub hitungproses()
        ...
        If GetData_UserSecurity("SELECT * FROM tb_userutility WHERE username = '" & LblUserName.Text & "' and hakakses = 'FORM HRD -- PENILAIAN BAWAHAN'") = 1 Then
            ...
            If LblUserName.Text = TxtHead.Text Then
                Call GetDataProses("select KPI_KODEITEMM,(sum((ISNULL(KPI_STAFF,0)) + (ISNULL(KPI_ATASAN,0))) / 2) / count(KPI_KODEITEMM) as NIlaiTotal from TRXN_KPIDD where  KPI_tipeA='Z' and KPI_NIK='" & TxtStaffNIK.Text & "' AND KPI_TAHUN='" & TxtKpiSummaryTahun.Text & "' AND KPI_KODEITEMM ='" & LblProsesKodeItem.Text & "' group by KPI_KODEITEMM")
    ...
    Protected Sub BtnKpi42BSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnKpi42BSave.Click
        If GetData_UserSecurity("SELECT * FROM tb_userutility WHERE username = '" & LblUserName.Text & "' And hakakses = 'FORM HRD -- PENILAIAN BAWAHAN'") = 1 And LblUserName.Text = TxtHead.Text Then
