...
Partial Class ISOSOP
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If LblUserName.Text = "" Then
            ...
            If GetData_UserSecurity("SELECT * FROM tb_user inner join tb_userutility on tb_user.username collate SQL_Latin1_General_CP1_CI_AS = tb_userutility.username collate SQL_Latin1_General_CP1_CI_AS WHERE tb_user.username = '" & LblUserName.Text & "' And tb_userutility.hakakses = 'FORM SOP -- ADD+DISTRIBUSI DATA'") = 1 Then
                ...
                Call ActiveButton()
            ElseIf GetData_UserSecurity("SELECT * FROM tb_user inner join tb_userutility on tb_user.username collate SQL_Latin1_General_CP1_CI_AS = tb_userutility.username collate SQL_Latin1_General_CP1_CI_AS WHERE tb_user.username = '" & LblUserName.Text & "' And tb_userutility.hakakses = 'FORM SOP -- VIEW DATA'") = 1 Then
            ...
            Else
                ...
                MultiViewEditSOP.ActiveViewIndex = -1
    ...
    Protected Sub BtnMasterTabel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnMasterTabel.Click
        If GetData_UserSecurity("SELECT * FROM tb_userutility WHERE username = '" & LblUserName.Text & "' and hakakses = 'FORM SOP -- ADD DATA' OR hakakses = 'FORM SOP -- ADD+DISTRIBUSI DATA'") = 1 Then
            LvDetailMaster.DataBind()
    ...
    'Event simpan ketika upload dokumen SOP baru
    Protected Sub BtnNilaiSMSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnNilaiSMSave.Click
        If GetFindRecord_Server("SELECT * FROM TRXN_DOKUMEN WHERE DOKUMEN_NO='" & lblBackupId.Text & "'") <> 1 Then
            ...
            If status.Text = " Baru" Then
                ...
                End If
                LvDetailMaster.DataBind()
                Response.Write("<script>alert('Dokumen SOP berhasil ditambah!')</script>")
                Response.Write("<script>window.location.href='ISOSOP.aspx';</script>")
            ElseIf status.Text = " Obsolete Dokumen" Then
                Dim filereceived As String = FileUpload1.PostedFile.FileName
                Dim filename As String = Path.GetFileName(filereceived)
                Call UpdateData_Server("INSERT INTO TRXN_DOKUMENO (DOKUMENO_NAMA,DOKUMENO_TGLTIDAKBERLAKU,DOKUMENO_LINK,DOKUMENO_DIBUAT,DOKUMENO_JENIS,DOKUMENO_KETERANGAN) VALUES ('" & TxtNamaDokumen.Text & "','" & Now() & "','" & filename & "', '" & LblUserName.Text & "','B','" & txtKeterangan.Text & "')")
                Dim fileuploadpath As String = Server.MapPath("~/WEBDOWNLOAD/DOKUMENISO/")
                FileUpload1.PostedFile.SaveAs(Path.Combine(fileuploadpath, filename))
                ListObsolete.DataBind()
                Response.Write("<script>alert('Dokumen Obsolete SOP berhasil ditambah!')</script>")
                MultiViewUploadDokumen.ActiveViewIndex = -1
                MultiViewAkses.ActiveViewIndex = -1
                MultiViewDistribusiDokumen.ActiveViewIndex = -1
    ...
    Private Sub UpdateData_Server(p As Object)
    ...
    Protected Sub BtnNilaiSM2Save_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnNilaiSM2Save.Click
        ...
        End If
        LvDetailMaster.DataBind()
        Response.Write("<script>alert('Dokumen SOP berhasil didistribusikan!')</script>")
        Response.Write("<script>window.location.href='ISOSOP.aspx';</script>")
    End Sub

    'Even Simpan ketika melakukan Update Dokumen
    Protected Sub BtnNilaiSM3Save_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnNilaiSM3Save.Click
        If GetFindRecord_Server("SELECT * FROM TRXN_DOKUMEN WHERE DOKUMEN_NO='" & lblBackupId.Text & "'") = 1 Then
            ...
            'Call KirimPesan() : Exit Sub ****memang di nonaktifkan
        End If
        LvDetailMaster.DataBind()
        Response.Write("<script>alert('Dokumen SOP berhasil diperbarui!')</script>")
        MultiViewUploadDokumen.ActiveViewIndex = -1
        MultiViewAkses.ActiveViewIndex = -1
        MultiViewDistribusiDokumen.ActiveViewIndex = -1
        MultiViewEditSOP.ActiveViewIndex = 0
        MultiViewFormEditDokumen.ActiveViewIndex = -1
        MultiViewObsolete.ActiveViewIndex = -1
    End Sub
    ...
    Protected Sub LvDetailMaster_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles LvDetailMaster.SelectedIndexChanged
        If GetData_UserSecurity("SELECT * FROM tb_userutility WHERE username = '" & LblUserName.Text & "' and hakakses = 'FORM SOP -- ADD+DISTRIBUSI DATA'") = 1 Then
            ...
            If CountData_Master("SELECT * FROM TRXN_DOKUMEN WHERE DOKUMEN_NO='" & lblBackupId.Text & "' AND DOKUMEN_DISETUJUI IS NULL OR DOKUMEN_DISETUJUI=''") = 1 Then
                ...
                Response.Write("<script>alert('Dokumen Standard Operasional Prosedur (SOP)  Belum Disetujui Direksi!')</script>") : Exit Sub
            Else
    ...
    'Event ketika menekan tombol distribusi dokumen (List Departtemen)
    Protected Sub LvDetailDept_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles LvDetailDept.SelectedIndexChanged
        ...
        'Response.Write("<script>alert('Departemen yang di pilih : '" + lblBackupId.Text + "'')</script>")
        If GetData_UserSecurity("SELECT * FROM tb_user inner join tb_userutility on tb_user.username collate SQL_Latin1_General_CP1_CI_AS = tb_userutility.username collate SQL_Latin1_General_CP1_CI_AS WHERE tb_user.username = '" & LblUserName.Text & "' And tb_userutility.hakakses = 'FORM SOP -- ADD+DISTRIBUSI DATA'") = 1 Then
    ...
    Protected Sub BtnMasterSop_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnMasterSop.Click
        If GetData_UserSecurity("SELECT * FROM tb_user inner join tb_userutility on tb_user.username collate SQL_Latin1_General_CP1_CI_AS = tb_userutility.username collate SQL_Latin1_General_CP1_CI_AS WHERE tb_user.username = '" & LblUserName.Text & "' And tb_userutility.hakakses = 'FORM SOP -- ADD+DISTRIBUSI DATA'") = 1 Then
            MultiViewUploadDokumen.ActiveViewIndex = -1
            MultiViewAkses.ActiveViewIndex = -1
            MultiViewDept.ActiveViewIndex = 0
            ...
            MultiViewEditSOP.ActiveViewIndex = -1
            MultiViewFormEditDokumen.ActiveViewIndex = -1
        ElseIf GetData_UserSecurity("SELECT * FROM tb_user inner join tb_userutility on tb_user.username collate SQL_Latin1_General_CP1_CI_AS = tb_userutility.username collate SQL_Latin1_General_CP1_CI_AS WHERE tb_user.username = '" & LblUserName.Text & "' And tb_userutility.hakakses = 'FORM SOP -- ADD DATA'") = 1 Then
            MultiViewSOP.ActiveViewIndex = 0
            MultiViewUploadDokumen.ActiveViewIndex = -1
            MultiViewEditSOP.ActiveViewIndex = -1
            MultiViewFormEditDokumen.ActiveViewIndex = -1
