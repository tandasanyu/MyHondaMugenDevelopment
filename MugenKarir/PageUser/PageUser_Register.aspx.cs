...
public partial class PagesUser_PageUser_Register : System.Web.UI.Page
{
    ...
    protected void BtnDaftar_Click(object sender, EventArgs e)
    {
        ...
        if (Validate())
        {
            ...
            bool cekdata = cn.KelasKoneksi_CheckData("select * from Login_User  where Username_Login = '" + TxtUsername.Text + "' ");

            if (cekdata)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "redirect",
                "alert('Username yang Anda Masukan Sudah Terpakai, Masukan Username Lain'); window.location='" +
                Request.ApplicationPath + "../PageUser/PageUser_ListLowongan2.aspx';", true);
            }
            else
            {
                ...
                if (hasil_Login_User == "1")
                {
                    ...
                    //insert Data_Lamaran
                    string SqlCmd_InsertData_Lamaran = "Insert into Data_Lamaran (User_Nama,User_Posisi,Id_login) values ('" + Hdp[0].ToString() + "','" + Hdp[1] + "'," + Convert.ToInt32(Hdp[2]) + ")";
                    ...
                    if (hasil_Data_Lamaran == "1")
                    {
                        ...
                        //ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Anda Telah Berhasil Register, Cek email segera! !')", true);
                        //redirect ke home pelamar
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "redirect",
                        "alert('Anda Telah Berhasil Register, Cek email segera!'); window.location='" +
                        Request.ApplicationPath + "../Login.aspx';", true);
                        //Response.Write("<script>alert('Anda Telah Berhasil Register, Cek email segera!')</script>");
